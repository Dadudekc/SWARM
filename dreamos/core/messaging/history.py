...
class PersistentMessageHistory(MessageHistory):
        ...
        try:
            ...
            if history_file.exists():
                with open(history_file, "r") as f:
                    data = json.load(f)
                    self._history = [Message.from_dict(msg_data) for msg_data in data]
    ...
    def _save_history(self) -> None:
        ...
        try:
            ...
            with open(history_file, "w") as f:
                json.dump([msg.to_dict() for msg in self._history], f, indent=2)
    ...
    async def get_history(
        ...
        try:
            ...
            if start_time:
                filtered = [msg for msg in filtered if msg.timestamp >= start_time]

            if end_time:
                filtered = [msg for msg in filtered if msg.timestamp <= end_time]
    ...
    async def clear_history(self, agent_id: Optional[str] = None) -> None:
        """Clear message history for a specific agent or all agents."""
        if agent_id:
            self._history = [
                msg
                for msg in self._history
                if msg.from_agent != agent_id and msg.to_agent != agent_id
            ]
        else:
            self._history.clear()
        self._save_history()
