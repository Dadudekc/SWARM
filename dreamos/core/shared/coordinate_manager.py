...
from .coordinate_utils import validate_coordinates, has_duplicate_coordinates, regions_overlap, region_overlap
...
class CoordinateManager:
    ...
    def _transform_coordinate_dict(self, coords: Dict) -> Dict:
        ...
        if 'response_region' in coords:
            ...
            top_left = self._transform_coordinates(region['top_left']['x'], region['top_left']['y'])
            bottom_right = self._transform_coordinates(region['bottom_right']['x'], region['bottom_right']['y'])
...
def load_coordinates(config_path: str) -> Dict:
    ...
    try:
        ...
        logger.error(f"Coordinate config file not found: {config_path}")
        raise
    except json.JSONDecodeError as e:
        logger.error(f"Invalid JSON in coordinate config: {e}")
        raise
...
def get_coordinates(agent_id: str = None) -> dict:
    ...
    Returns:
        Dictionary of coordinates
    """
    coords_file = Path(__file__).parent.parent.parent / "config" / "cursor_agent_coords.json"
