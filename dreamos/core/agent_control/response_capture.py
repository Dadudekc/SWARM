    ...
    def capture_response(self, agent_id: str, timeout: int = 30) -> Optional[str]:
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                # Log region details
                logger.debug(f"[ResponseCapture] Capturing region: x={x}, y={y}, w={width}, h={height}")
                ...
                if x < 0 or y < 0 or x + width > screen_width or y + height > screen_height:
                    logger.error(f"Region is off-screen: x={x}, y={y}, w={width}, h={height}, screen={screen_width}x{screen_height}")
                    return None
                ...
                if img_array.size == 0:
                    logger.error("[ResponseCapture] Empty image array — region may be off-screen")
                    return None
                
                logger.debug(f"[ResponseCapture] Captured image size: {img_array.shape}")
                ...
                if text:
                    if text == last_text:
                        unchanged_count += 1
                        if unchanged_count >= 3:  # Text unchanged for 3 consecutive checks
                        ...
                        if text:
                            if text == last_text:
                                ...
                                if unchanged_count >= 3:
                                    logger.info(f"Response captured for {agent_id} (direct OCR)")
                                    return text
                            else:
                                unchanged_count = 0
                                last_text = text
