...
class AgentRestarter:
    ...
    def __init__(
        ...
        self.timing = {
            ...
            "delay_after_window_activate": self.config.get(
                "delay_after_window_activate", 0.5
            ),
        }
    ...
    async def _load_coordinates(self) -> None:
        ...
        try:
            ...
            coords_data = load_coordinates(self.coords_file)
            ...
            for agent_id, coords in coords_data.items():
                if isinstance(coords, (list, tuple)) and len(coords) == 2:
                    ...
                    logger.warning(
                        f"Invalid coordinates for agent {agent_id}: {coords}"
                    )
    ...
    async def _get_initial_prompt(self, agent_id: str, resume_reason: str) -> str:
        ...
        try:
            ...
            # Apply substitutions
            prompt = prompt.format(agent_id=agent_id, resume_reason=resume_reason)
    ...
    async def _restart_conversation(
        self, agent_id: str, resume_reason: str = "timeout"
    ) -> bool:
    ...
    async def handle_agent_error(
        self, agent_id: str, resume_reason: str = "error"
    ) -> bool:
        ...
        try:
            ...
            # Attempt recovery
            logger.info(
                f"Attempting recovery for agent {agent_id} (attempt {retry_count})"
            )
